...
exports.displayNutritionCards = function getNutritionData(foodName, session){ //export to use inside LUIS file
    var url = "https://api.nal.usda.gov/ndb/search/?format=json&q="+foodName+"&sort=r&max=1&offset=0&api_key=ml711G78Wwur9LjSAfU8LjMoe7uf9cRuq9mvD7Dw";
...
function getFoodNutrition(message, foodName, session){ //Pass message in
    var foodNutritionList = JSON.parse(message);
    var ndbno = foodNutritionList.list.item[0].ndbno; //ndbno = nutrition database number
    var url = "https://api.nal.usda.gov/ndb/reports/?ndbno="+ndbno+"&type=f&format=json&api_key=ml711G78Wwur9LjSAfU8LjMoe7uf9cRuq9mvD7Dw";
    
    rest.getNutritionData(url, session, foodName, displayNutritionCards); //displayNutritionCard = callback function
